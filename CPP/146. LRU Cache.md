...
* 第一个是我自己写的，每次在list尾部存储，这样list的第一个元素就是LRU,每次put新元素是插到list尾
* map里的保存的是{key, {value, 可以在time_list中对应的iterator}}
* time_list保存的是key进入的顺序,time_list.begin()就是LRU
...
public:
    ...
    std::list<int> timelist; // the sequence of the putting: the first node is the LRC,保存的是key,最主要是在这里存储
    int capacity;
};
```
